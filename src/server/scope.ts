...
import { type OC, OCError, type Session } from "./oc";
...
export async function workbenchSession(oc: OC, config: Config, id: string): Promise<Session> {
  let session: Session;
  try {
    session = await oc.sessions.get(id);
  } catch (cause) {
    if (cause instanceof OCError && cause.status === 404) throw new ScopeError(id);
    throw cause;
  }
