...
- `src/shared/` the seam the client and the server both import and nothing else crosses: `task.ts` the `Task` type, its execution facet, the label keys and bounds; `report.ts` the report parser built from the tool's schema and the result's stage; `problem.ts` the one error shape (`dev/test/seams.test.ts` enforces the rule)
