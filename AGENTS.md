...
- `src/lib/` the browser's own modules: `api.ts` its view of the app's routes, `submission.ts` the envelope held until the admission receipt, `ulid.ts`, `activity.ts` the hook's turns joined with event notes (`STOPGAP(C5)`), `vocabulary.ts` the display state, the words and the failure copy, `theme.ts`, `format.ts`
