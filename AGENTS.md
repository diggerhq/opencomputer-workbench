...
- `opencomputer/` the agent: `project.ts` names the project and its one agent; `agents/worker/agent.ts` the worker (the GitHub connection, the computer's tools, one tool of its own, one model, its instructions); `report.schema.ts` the one source of the result type, the JSON Schema the tool declares and the app parses with; `tools/report.ts` the report tool, its claims verified against GitHub; `read-task.ts` what the agent knows about its task
- `src/routes/` every route of the application, one file per path: `__root.tsx` the document and the shell, `index.tsx` the list, `tasks.$id.tsx` the task page; under `api/` and `auth/` the server routes (`workspace`, `repos`, `tasks`, `tasks.$id`, `tasks.$id.end`, `agent/sessions.$id.$action` the three routes the React hook needs, `auth/login`, `callback`, `logout`); `-guards.ts` the guards they compose: same-origin, member (which provides the configuration, the client and the clock) and task (which provides the scope-checked session)
...
- `src/shared/` the seam the client and the server both import and nothing else crosses: `task.ts` the `Task` type, its execution facet, the label keys and bounds; `report.ts` the report parser built from the agent's schema, the result's stage and `TaskResult`, the result with its provenance; `problem.ts` the one error shape (`dev/test/seams.test.ts` enforces the rule)
- `src/lib/` the client side's own modules, imported by the components and the page routes only: `api.ts` the browser's view of the app's routes, `submission.ts` the envelope held until the admission receipt, `ulid.ts`, `activity.ts` the hook's turns joined with the event notes, `activity-notes.ts` those notes (`STOPGAP(C5)`, one deletable file), `command-output.ts` the shell tool's output as an outcome, `vocabulary.ts` the display state, the words and the failure copy, `theme.ts`, `format.ts`
...
- `docs/setup.md` the setup guide the README links, and `components.json`, which the shadcn generator reads from the project directory: the two files outside `dev/` that are not the example itself
