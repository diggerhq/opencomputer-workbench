...
- `src/lib/` the browser's own modules: `api.ts` its view of the app's routes, `submission.ts` the envelope held until the admission receipt, `display.ts`, `ulid.ts`, `activity.ts` the hook's turns joined with event notes (`STOPGAP(C5)`), `vocabulary.ts` the words and the failure copy, `theme.ts`, `format.ts`
