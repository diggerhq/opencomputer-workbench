...
describe("the submission envelope", () => {
  it("mints one task id per logical submission and keeps it across retries", () => {
    ...
    expect(envelope).toMatchObject({ deploymentId: "dep_dev", repo: "acme/service", ref: "main", text: "Do it" });
    ...
    expect(compose({ repo: "a/b", ref: "main", text: "x" }, "dep_dev").taskId).not.toBe(envelope.taskId);
  });

  it("is held until a receipt arrives, a duplicate one included", () => {
    const state = begin(compose({ repo: "acme/service", ref: "main", text: "Do it" }, "dep_dev"));
    const done = succeed(state, { task, receipt: { ...receipt, duplicate: true } });
    expect(done).toMatchObject({ status: "done", task, receipt: { duplicate: true } });
    expect(succeed(IDLE, { task, receipt })).toBe(IDLE);
  ...
  it("retries lost replies and outages by itself, a bounded number of times", () => {
    let state = begin(compose({ repo: "acme/service", ref: "main", text: "Do it" }, "dep_dev"));
