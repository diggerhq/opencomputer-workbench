...
describe("the route table", () => {
  ...
  it("describes the workspace for a member", async () => {
    ...
    // Bootstrapping the browser asks the platform nothing: which deployment
    // runs a task is decided when the task starts, by the platform.
  ...
  it("forwards OpenComputer's error code and clears nothing", async () => {
    ...
    configure({ config: cfg, fetch, now: () => T0 });
    const response = await serve(new Request("https://workbench.example/api/tasks", { headers: await member() }));
